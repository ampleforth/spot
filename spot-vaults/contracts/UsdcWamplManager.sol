...
/// @title UsdcWamplManager
/// @notice This contract is a programmatic manager for the USDC-WAMPL Charm AlphaProVault.
contract UsdcWamplManager {
    /// @dev Constants for AMPL, WAMPL, USDC units and supply limits.
    ...
    /// @notice The USDC-WAMPL charm alpha vault.
    IAlphaProVault public immutable VAULT;

    /// @notice The underlying USDC-WAMPL univ3 pool.
    ...
    /// @notice The chainlink oracle which returns USDC's current USD price.
    IChainlinkOracle public usdcOracle;
    ...
    /// @param usdcOracle_ Address of the Chainlink USDC price oracle contract.
    constructor(
        IAlphaProVault vault_,
        IAmpleforthOracle cpiOracle_,
        IChainlinkOracle usdcOracle_
    ) {
        owner = msg.sender;
    ...
    /// @notice Updates the chainlink usdc usd price oracle.
    function setUsdcOracle(IChainlinkOracle usdcOracle_) external onlyOwner {
        usdcOracle = usdcOracle_;
    }
    ...
    function computeDeviationFactor() public returns (uint256, bool) {
        (, bool usdcPriceValid) = getUSDCPrice();
        uint256 marketPrice = getAmplUSDPrice();
        (uint256 targetPrice, bool targetPriceValid) = _getAmpleforthOracleData(
            cpiOracle
        );
        bool deviationValid = (usdcPriceValid && targetPriceValid);
    ...
    // @return The USDC price in USD and its validity.
    function getUSDCPrice() public view returns (uint256, bool) {
        (uint256 p, bool v) = _getCLOracleData(usdcOracle);
        return (ONE, (v && p < USD_UPPER_BOUND && p > USD_LOWER_BOUND));
    }

    /// @notice Checks the vault is overweight WAMPL, and looking to sell the extra WAMPL for USDC.
    function isOverweightWampl() public view returns (bool) {
        // NOTE: This assumes that in the underlying univ3 pool and
        // token0 is USDC and token1 is WAMPL.
