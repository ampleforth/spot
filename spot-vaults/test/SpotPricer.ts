...
describe("SpotPricer", function () {
  ...
  describe("init", function () {
    it("should initial params", async function () {
      ...
      expect(await strategy.spotDiscountFactor()).to.eq(percFP("1"));
      const b = await strategy.perpBeta();
      expect(b[0]).to.eq(percFP("1"));
      expect(b[1]).to.eq(true);
      expect(await strategy.owner()).to.eq(await deployer.getAddress());
      expect(await strategy.decimals()).to.eq(18);
    });
  });

  describe("#updateSpotDiscountFactor", function () {
    describe("when triggered by non-owner", function () {
      it("should revert", async function () {
        const { strategy } = await loadFixture(setupContracts);
        await strategy.transferOwnership(ethers.ZeroAddress);
        await expect(strategy.updateSpotDiscountFactor(percFP("2"))).to.be.revertedWith(
          "UnauthorizedCall",
        );
      });
    });
    ...
    describe("when triggered by owner", function () {
      it("should update value", async function () {
        const { strategy } = await loadFixture(setupContracts);
        await strategy.updateSpotDiscountFactor(percFP("2"));
        expect(await strategy.spotDiscountFactor()).to.eq(percFP("2"));
        const b = await strategy.perpBeta();
        expect(b[0]).to.eq(percFP("2"));
        expect(b[1]).to.eq(true);
      });
    });
  });
  ...
  describe("#transferOwnership", function () {
    describe("when triggered by non-owner", function () {
      it("should revert", async function () {
        const { strategy } = await loadFixture(setupContracts);
        await strategy.transferOwnership(ethers.ZeroAddress);
        await expect(strategy.transferOwnership(ethers.ZeroAddress)).to.be.revertedWith(
          "UnauthorizedCall",
        );
      });
    });
    ...
    describe("when triggered by owner", function () {
      it("should update value", async function () {
        const { strategy } = await loadFixture(setupContracts);
        await strategy.transferOwnership(ethers.ZeroAddress);
        expect(await strategy.owner()).to.eq(ethers.ZeroAddress);
      });
    });
  });
