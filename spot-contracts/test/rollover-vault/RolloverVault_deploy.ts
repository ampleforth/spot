  ...
  describe("#deploy", function () {
    describe("when usable balance is zero", function () {
      it("should revert", async function () {
        await expect(vault.deploy()).to.be.revertedWithCustomError(vault, "LiquidityOutOfBounds");
      });
    });

    describe("when minUnderlyingBal is not set", function () {
      beforeEach(async function () {
        await vault.updateLiquidityLimits(toFixedPtAmt("0"), "0", ethers.MaxUint256);
      });
    ...
    describe("when minUnderlyingBal is set", function () {
      beforeEach(async function () {
        await vault.updateLiquidityLimits(toFixedPtAmt("25"), "0", ethers.MaxUint256);
      });
    ...
    describe("when one trancheIn one tokenOut (near mature tranche)", function () {
      ...
      describe("when balance covers just 1 token", function () {
        it("should rollover", async function () {
          ...
          await checkVaultComposition(
            vault,
            [collateralToken, newTranchesIn[1], curTranchesIn[0], curTranchesIn[1], reserveTranches[3]],
            [
              toFixedPtAmt("6599.999999950000000000"),
              toFixedPtAmt("3200"),
      ...
      describe("when balance covers just 1 token exactly", function () {
        ...
        it("should rollover", async function () {
          ...
          await checkVaultComposition(
            ...
            [
              toFixedPtAmt("15099.999999950000000000"),
              toFixedPtAmt("200"),
              toFixedPtAmt("3200"),
              toFixedPtAmt("0.00000001"),
              toFixedPtAmt("0.00000004"),
    ...
    describe("when rollover fee is +ve", function () {
      beforeEach(async function () {
        await balancer.mockMethod("computeRolloverFeePerc(uint256)", [toPercFixedPtAmt("0.01")]);
