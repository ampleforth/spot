    ...
    describe("when fee > zero", function () {
      ...
      describe("when spot > stampl", function () {
        ...
        it("should return rebalanced tokens", async function () {
          ...
          await txFn();
          expect(r[0][0]).to.eq(toFixedPtAmt("22.623042505592840710"));
          expect(r[0][1]).to.eq(toFixedPtAmt("112361111.111111108776360295"));

          const perpTokens = r[1];
          expect(perpTokens[0].token).to.eq(perpTranches[3].target);
          expect(perpTokens[0].amount).to.eq(toFixedPtAmt("21.427852348993288770"));
          expect(perpTokens[1].token).to.eq(perpTranches[1].target);
          expect(perpTokens[1].amount).to.eq(toFixedPtAmt("21.427852348993288770"));
          expect(perpTokens[2].token).to.eq(perpTranches[2].target);
          expect(perpTokens[2].amount).to.eq(toFixedPtAmt("21.427852348993288770"));
        ...
        it("should re-mint perps", async function () {
          await expect(txFn).to.changeTokenBalances(perp, [deployer], [toFixedPtAmt("-77.376957494407159290")]);
        });

        it("should re-mint vault notes", async function () {
          await expect(txFn).to.changeTokenBalances(vault, [deployer], [toFixedPtAmt("12361111.111111108776360295")]);
        });
        ...
        it("should transfer tranches", async function () {
          await expect(txFn).to.changeTokenBalances(
            perpTranches[1],
            [deployer],
            [toFixedPtAmt("21.427852348993288770")],
          );
        });
      ...
      describe("when spot < stampl", function () {
        ...
        it("should return rebalanced tokens", async function () {
          ...
          await txFn();
          expect(r[0][0]).to.eq(toFixedPtAmt("16.733780760626398210"));
          expect(r[0][1]).to.eq(toFixedPtAmt("83111111.111111111110567077"));
          ...
          const vaultTokens = r[2];
          expect(vaultTokens[0].token).to.eq(remTranches[3].target);
          expect(vaultTokens[0].amount).to.eq(toFixedPtAmt("3.251677852348993287"));
          expect(vaultTokens[1].token).to.eq(remTranches[1].target);
          expect(vaultTokens[1].amount).to.eq(toFixedPtAmt("3.251677852348993287"));
          expect(vaultTokens[2].token).to.eq(remTranches[2].target);
          expect(vaultTokens[2].amount).to.eq(toFixedPtAmt("3.251677852348993287"));
        });

        it("should re-mint perps", async function () {
          await expect(txFn).to.changeTokenBalances(perp, [deployer], [toFixedPtAmt("6.733780760626398210")]);
        });

        it("should re-mint vault notes", async function () {
          await expect(txFn).to.changeTokenBalances(vault, [deployer], [toFixedPtAmt("-16888888.888888888889432923")]);
        });
        ...
        it("should transfer tranches", async function () {
          await expect(txFn).to.changeTokenBalances(remTranches[1], [deployer], [toFixedPtAmt("3.251677852348993287")]);
        });

        it("should transfer tranches", async function () {
          await expect(txFn).to.changeTokenBalances(remTranches[2], [deployer], [toFixedPtAmt("3.251677852348993287")]);
        });

        it("should transfer tranches", async function () {
          await expect(txFn).to.changeTokenBalances(remTranches[3], [deployer], [toFixedPtAmt("3.251677852348993287")]);
        });
      ...
      describe("when spot = stampl", function () {
        ...
        it("should return redeemed tokens", async function () {
          const r = await balancer.rebalance.staticCall({
            perpAmt: toFixedPtAmt("15"),
            noteAmt: toFixedPtAmt("111750000"),
          });

          await txFn();
          expect(r[0][0]).to.eq(toFixedPtAmt("21.25"));
          expect(r[0][1]).to.eq(toFixedPtAmt("105541666.666666666666666666"));
        ...
        it("should re-mint perps", async function () {
          await expect(txFn).to.changeTokenBalances(perp, [deployer], [toFixedPtAmt("5.735123042505592841")]);
        });

        it("should re-mint vault notes", async function () {
          await expect(txFn).to.changeTokenBalances(vault, [deployer], [toFixedPtAmt("-5968888.888888888889765178")]);
        });
