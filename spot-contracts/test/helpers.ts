...
export const mintVaultNotesWithZ = async (vault: Contract, tranche: Contract, amount: Contract, from: Signer) => {
  const _balancer = await vault.balancer();
  await vault.updateBalancer(await from.getAddress());
  await tranche.connect(from).approve(vault.target, amount);
  const tx = await vault.connect(from)["deposit(address,uint256)"](tranche.target, amount);
  await vault.updateBalancer(_balancer);
  return tx;
};
