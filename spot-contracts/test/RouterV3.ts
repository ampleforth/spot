...
describe("RouterV3", function () {
  ...
  describe("#trancheAndMintPerps", function () {
    beforeEach(async function () {
    ...
    describe("when deposit bond is incorrect", function () {
      ...
      it("should revert", async function () {
        await expect(
          router.trancheAndMintPerps(balancer.target, depositBond.target, toFixedPtAmt("1000")),
    ...
    describe("when deposit bond is not issued", function () {
      ...
      it("should not revert", async function () {
        ...
        await expect(router.trancheAndMintPerps(balancer.target, depositBond.target, toFixedPtAmt("1000"))).not.to.be
          .reverted;
      });
    });
    ...
    describe("when deposit bond is correct", function () {
      ...
      beforeEach(async function () {
        await collateralToken.approve(router.target, ethers.MaxUint256);
        r = await router.trancheAndMintPerps.staticCall(balancer.target, depositBond.target, toFixedPtAmt("1000"));
        await router.trancheAndMintPerps(balancer.target, depositBond.target, toFixedPtAmt("1000"));
      });
