...
import { InvalidTargetSRBounds, InvalidPerc, InvalidSigmoidAsymptotes } from "./_interfaces/ProtocolErrors.sol";
...
contract Balancer is IBalancer, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function rebalance(
        ...
        // Remint perp and vault notes using underlying
        PairAmounts memory mintAmts = _execMint2(underlyingAmt, 0, 0);

        // Charge redemption fee and transfer redeemed tokens
        // Redemption fee is paid in kind to perp and the vault, by transferring some assets back
        uint256 perpFeePerc = fees.perpBurnFeePerc;
        for (uint8 i = 0; i < perpTokens.length; i++) {
            uint256 feeAmt = perpTokens[i].amount.mulDiv(perpFeePerc, ONE, MathUpgradeable.Rounding.Up);
            perpTokens[i].amount -= feeAmt;
            perpTokens[i].token.safeTransfer(address(perp), feeAmt);
            perp.sync(perpTokens[i].token);
            perpTokens[i].token.safeTransfer(msg.sender, perpTokens[i].amount);
        }
        uint256 vaultFeePerc = fees.vaultBurnFeePerc;
        for (uint8 i = 0; i < vaultTokens.length; i++) {
            uint256 feeAmt = vaultTokens[i].amount.mulDiv(vaultFeePerc, ONE, MathUpgradeable.Rounding.Up);
            vaultTokens[i].amount -= feeAmt;
            vaultTokens[i].token.safeTransfer(address(vault), feeAmt);
            vault.sync(vaultTokens[i].token);
            vaultTokens[i].token.safeTransfer(msg.sender, vaultTokens[i].amount);
        }

        // Transfer out minted perps and vault notes back to user
        perp.safeTransfer(msg.sender, mintAmts.perpAmt);
        vault.safeTransfer(msg.sender, mintAmts.noteAmt);
