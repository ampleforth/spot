...
interface IPerpetualTranche is IERC20 {
    ...
    // @notice Event emitted when a new bond is added to the queue.
    // @param strategy Address of the bond added to the queue.
    event BondEnqueued(IBondController bond);

    // @notice Event emitted when a bond is removed from the queue.
    ...
    // @param requestedAmount The amount of perp tokens requested to be redeemed.
    ...
    // @notice Iteratively dequeues bonds till the head of the queue has an active bond.
    // @return True if successful.
