...
contract PricingStrategy is Ownable, IPricingStrategy {
	...
	function getTranchePrice(IBondMinter minter, IBondController bond, uint256 seniorityIDX, uint256 trancheAmt) external view override returns (uint256) {
		uint256 yieldFactor = trancheYields(minter, bond, seniorityIDX) * computeTranchePrice(bond, seniorityIDX) / (10 ** PRICE_DECIMALS);
		return yieldFactor * trancheAmt / (10 ** PCT_DECIMALS);
	}

	function trancheYields(IBondMinter minter, IBondController bond, uint256 seniorityIDX) private view returns (uint256) {
